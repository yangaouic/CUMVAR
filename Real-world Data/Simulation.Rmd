...
#data generating 
y <- beta0 + beta*group + B0 + B1*time + B2*time^2 + epsilon
df3 <- data.frame(ID = ID, time = time, group = group, y = y, t2 = time^2/1000)

#fitting model
lme.fit <- lmer(y ~ group + (1 + time + t2 | ID), data = df3)
summary(lme.fit)
```
...
### spline on random effect
spline on random effect with fixed intercept
```{r}
set.seed(1234)
n <- 10000; obs <- 40
...
spline on random effect with random intercept
```{r}
set.seed(1234)
n <- 10000; obs <- 40
time <- rweibull(n*obs, 2)
...
Cumulative variation and plots
```{r}
#predict data
long_time <- c()
ID <- c()
